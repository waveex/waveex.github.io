...
console.log(
  `Cześć, na tej stronie znajdziesz informacje o moich projektach, mam nadzieję że Ci się podoba`
);
...
fetch("https://api.github.com/users/waveex/repos?sort=updated&direction=desc")
  ...
  .then((resp) => {
    ...
    for (const repo of repos) {
      ...
      if (name !== "waveex.github.io" || name !== "waveex") {
        list.innerHTML += `
      <li class="projects-list__item">
      <img class="projects-list__img" src="../assets/img/github.svg" alt="" />
      <a class="projects-list__link" a >${name}</a>
      <p class="projects-list__link--description">${
        description ? description : "work in progress demo coming soon 👨‍💻"
      }</p>
...
<a href="${html_url}" target="_blank" rel="nofollow noreferrer"  class="projects-list__footer--script projects-list__footer--script-js">GITHUB</a>
   ${
     homepage
       ? `<a href="${homepage}" target="_blank" rel="nofollow noreferrer" class="projects-list__footer--demo projects-list__footer--demo-js">DEMO</a>`
       : ``
   }
      </div>
    </li>
